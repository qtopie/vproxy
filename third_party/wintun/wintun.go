...
func logMessage(level loggerLevel, timestamp uint64, msg *uint16) int {
	if msg == nil {
		log.Printf("wintun: received log message with a nil payload (level=%d, timestamp=%d)", level, timestamp)
		return 0
	}
...
func setupLogger(dll *lazyDLL) {
	// The legacy callback signature is incompatible with newer Wintun DLLs
	// and can crash while the adapter is being created. Wintun logging is
	// optional, so leave the DLL logger at its default instead.
}
