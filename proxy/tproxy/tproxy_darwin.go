		...
		for {
			...
			for i := 0; i < n; i++ {
				...
				pkt := stack.NewPacketBuffer(stack.PacketBufferOptions{
					Payload: buffer.MakeWithData(packet[offset : offset+sizes[i]]),
				})
				// Determine protocol from IP header
				proto := ipv4.ProtocolNumber
				if sizes[i] > 0 && (packet[offset]>>4) == 6 {
...
func setupRouting(tunName string) error {
	...
	// 2. Add routes
	// route add -net 0.0.0.0/1 -interface utunX
	// route add -net 128.0.0.0/1 -interface utunX
	cmds := [][]string{
